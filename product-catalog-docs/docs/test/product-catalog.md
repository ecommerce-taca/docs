...
| PC-INT-006b *(2026-09-19, `DOCS-CDC-02`)* | Outbox | Field vận chuyển cho connector (`topic`/`version`/`actor_user_id`/`traceparent`) đúng theo từng nhóm event | Publish product (seller action) → `topic="product.events.v1"`, `version` khớp `products.version` sau mutation, `actor_user_id` = user thực hiện, `traceparent` = trace của request. Consume shop event khiến `product.shop_snapshot_updated` phát ra (projection-driven) → cùng field nhưng `actor_user_id=null`. Sai `topic` (không khớp cột Topic ở `lld` §6.2 cho đúng `event_type`) phải bị bắt ở review/contract test, không chỉ runtime. |
| PC-INT-006c *(2026-09-19, `DOCS-CDC-02`)* | Outbox | Đối chiếu đủ 15 event type × `topic` dự kiến | Test data-driven: với mỗi `event_type` ở `lld` §6.2, publish/trigger rồi assert `outbox_events.topic` đúng 1-trong-4 topic đã chốt — không event nào lệch topic (chặn regression kiểu RB1). |
...
| `OutboxPublisher` | Write-only: ghi record đúng schema/aggregate key trong cùng transaction với domain mutation; **không** test retry/backoff/DLQ ở unit này (thuộc connector, không phải class này); redaction payload (không leak secret/KYC document); envelope có `version` cho consumer order; ghi đúng `topic` theo `event_type` (map 1-1 với `lld` §6.2); `actor_user_id` lấy từ request context khi có, `null` khi event sinh ra từ việc consume event khác; `traceparent` lấy từ active trace context, `null` nếu không có (background job). |
