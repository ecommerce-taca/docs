...
| Request ID/trace | `X-Request-ID`, W3C `traceparent`/`tracestate` (do Gateway/service propagate, client không gửi); socket event có trace metadata. |
...
| Pagination | List dùng `page` từ 1, `size` mặc định 20 tối đa 100, meta `request_id,page,size,total,total_pages`; **history message dùng cursor** (`has_more`, `next_after_sequence`, tối đa 100). |
...
| 3 | `GET /conversations/{conversationId}` | Participant/support | Detail/participants/context. |
| 4 | `GET /conversations/{conversationId}/messages` | Participant/support | Message history cursor. |
| 5 | `POST /conversations/{conversationId}/messages` | Participant/support | Gửi message REST. |
| 6 | `PATCH /messages/{messageId}` | Sender | Sửa message trong window. |
| 7 | `DELETE /messages/{messageId}` | Sender | Soft delete. |
| 8 | `PATCH /conversations/{conversationId}/read` | Participant | Mark read cursor. |
| 9 | `POST /conversations/{conversationId}/attachments/upload-url` | Participant | Signed upload. |
| 10 | `POST /attachments/{attachmentId}/complete` | Uploader | Complete metadata. |
...
- `GET /conversations/{conversationId}`: trả participants safe, context label/reference, last sequence/time, unread count; không trả private account data.
...
```json
{
  ...
  "meta": { "request_id": "01912fc1-7a1b-7c12-9c55-8b1c34a6d921", "page": 1, "size": 20, "total": 5, "total_pages": 1 }
}
```
...
`GET /conversations/{conversationId}/messages?after_sequence=&before_sequence=&limit=` — cursor theo `sequence` tăng dần trong conversation (`limit` mặc định 50, max 100). Reconnect WebSocket dùng `after_sequence` để lấy phần bị miss.
...
- `PATCH /messages/{messageId}` body `{version, body, attachment_ids[]}` — chỉ sender, trong cửa sổ sửa 15 phút; hết hạn → `409 MESSAGE_EDIT_EXPIRED`.
- `DELETE /messages/{messageId}` body `{version}` — soft delete, giữ `sequence`, trả placeholder `deleted:true` và `body:null` cho mọi participant.

### 3.3 Read/realtime

`PATCH /conversations/{conversationId}/read` body:
...
`POST /attachments/{attachmentId}/complete` body `{object_key,sha256}`; HEAD/checksum/type/scan. Response `200`:
...
`READY` mới dùng được cho `attachment_ids` ở `POST /conversations/{conversationId}/messages` (§3.2); `SCANNING` chưa public, gửi kèm message sẽ bị `400 MESSAGE_ATTACHMENT_INVALID`. Sai checksum/type/size → `status: "REJECTED"`, không retry `complete` với cùng `attachment_id` — phải upload lại.
